...
public class LoginCmdHandler extends SimpleChannelInboundHandler<LoginCmd> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, LoginCmd msg) {
        ...
        //确定配置中存在此客户端名
        if (serverInfo == null) {
            ctx.writeAndFlush(new ServerStartFailCmd(0, "", 0, "无法识别客户端名:" + msg.getClientName())).addListener(ChannelFutureListener.CLOSE);
            return;
        }
        //确认此消息的token是正确的
        try {
            ...
            ctx.writeAndFlush(new ServerStartFailCmd(0, "", 0, "token验证不通过")).addListener(ChannelFutureListener.CLOSE);
            return;
        }
